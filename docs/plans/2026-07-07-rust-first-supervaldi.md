...
| 05 | `codex/rust-supervaldi-05-codec-validator` | feat: add IR codec validator inspector | [05-codec-validator.md](2026-07-07-rust-first-supervaldi/steps/05-codec-validator.md) | Add production and debug encoding plus validation tooling. | PR04 | Rust | Every fixture round-trips through JSON and binary codecs. | Complete |
| 06 | `codex/rust-supervaldi-06-backend-ops` | feat: add full backend operation model | [06-backend-ops.md](2026-07-07-rust-first-supervaldi/steps/06-backend-ops.md) | Create the backend operation contract shared by native, web, PNG, and transition renderers. | PR05 | Rust | Mock backend snapshots cover every operation family. | Complete |
| 07 | `codex/rust-supervaldi-07-runtime-tree-diff` | feat: add Rust runtime tree diff | [07-runtime-tree-diff.md](2026-07-07-rust-first-supervaldi/steps/07-runtime-tree-diff.md) | Add Rust document loading, identity preservation, and render-op diffing. | PR06 | Rust | Static fixture declarations emit expected ordered backend ops. | Complete |
| 08 | `codex/rust-supervaldi-08-state-bindings-actions` | feat: add state bindings and action scheduler | [08-state-bindings-actions.md](2026-07-07-rust-first-supervaldi/steps/08-state-bindings-actions.md) | Add Rust state, binding evaluation, and action dispatch. | PR07 | Rust | State fixtures update predictably. | Complete |
| 09 | `codex/rust-supervaldi-09-rust-ui-dsl` | feat: add complete Rust UI DSL | [09-rust-ui-dsl.md](2026-07-07-rust-first-supervaldi/steps/09-rust-ui-dsl.md) | Add the Rust authoring surface for declaring UI. | PR08 | Rust | DSL output equals canonical IR for every contract row. | Complete |
| 10 | `codex/rust-supervaldi-10-tsx-to-ir-compat` | feat: add TSX-to-IR compatibility frontend | [10-tsx-to-ir-compat.md](2026-07-07-rust-first-supervaldi/steps/10-tsx-to-ir-compat.md) | Preserve existing TypeScript apps and surface schema gaps before backend replacement begins. | PR09 | TS compatibility + Rust IR | TSX fixtures produce normalized IR equal to Rust DSL fixtures. | Complete |
| 11 | `codex/rust-supervaldi-11-ir-hot-reload` | feat: add IR hot reload pipeline | [11-ir-hot-reload.md](2026-07-07-rust-first-supervaldi/steps/11-ir-hot-reload.md) | Deliver fast UI hot reload through IR patches. | PR10 | Rust tooling + debug host hooks | UI tree, style, layout, text, asset, binding, event, accessibility, module ref, and native view ref edits patch live on the sample host without reinstall or relaunch. | Complete |
| 12 | `codex/rust-supervaldi-12-rust-hot-patch` | feat: add Rust logic hot patch support | [12-rust-hot-patch.md](2026-07-07-rust-first-supervaldi/steps/12-rust-hot-patch.md) | Add the declared Rust logic edit class for live development. | PR11 | Rust tooling | Supported action-body edit patches live. | Complete |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Contract misses a production surface | Rust apps and backends | Architecture churn and missing replacement capability | High | PR01 machine-readable contract references repo facts and owner/proof for every surface | PR03, PR04, PR05, and PR36 consume the contract directly | 01 | Open |
...
| Hot reload requires full Rust rebuild for common UI edits | Developer workflow | Slow iteration | High | PR11 parses declarative DSL into IR without full Rust compilation for UI edits | Hot reload latency report and fixture edits | 11 | Mitigated by PR11 sample-host proof; PR12 still owns action-body hot patch |
| Rust logic hot patch claims too much | Developer workflow | Incorrect live code behavior | Medium | PR12 supports only declared action-body edit class and returns rebuild-required for every other class | Support matrix suite | 12 | Mitigated by PR12 support matrix and exact rebuild-required diagnostics |
...
| TS compatibility becomes hidden Rust dependency | Architecture | Rust path remains split-brain | Medium | PR10 keeps TSX-to-IR in `compiler/companion` compatibility code and proves Rust crate graph/source paths stay TS-free; PR38 checks final Rust app graph | Bazel graph check | 10,38 | Open; PR10 proof added |
...
| 05 | Complete | Every fixture round-trips through JSON and binary codecs. | Added the typed JSON debug codec, postcard binary codec, contract-bound validator, exact invalid diagnostic checks, and `ir-inspect` snapshot proof. |
| 06 | Complete | Mock backend snapshots cover every operation family. | Added typed backend operations, capability validator, mock backend snapshot, fixture-tag coverage proof, and configured Rust target compile checks. |
| 07 | Complete | Static fixture declarations emit expected ordered backend ops. | Added typed runtime document loading, identity tables, tree diff operation emission, state-preserving patch checks, rebuild diagnostics, and static backend-op snapshot proof. |
| 08 | Complete | State fixtures update predictably. | Added typed state store, binding evaluator, action scheduler, event-action binding table, retained state compatibility checks, and state/binding/action trace snapshot proof. |
| 09 | Complete | DSL output equals canonical IR for every contract row. | Added `valdi_rust_dsl` app-facing builders, exact invalid DSL diagnostics, contract/fixture-backed golden proof, Rust app sample compile proof, and public API forbidden dependency checks. |
| 10 | Complete | TSX fixtures produce normalized IR equal to Rust DSL fixtures. | Added TSX normalized-IR sidecar, retained JSXProcessor regression, retained TS sample `.valdimodule` build proof, contract/fixture-backed coverage manifest, and Rust graph no-TS-dependency proof. |
| 11 | Complete | UI tree, style, layout, text, asset, binding, event, accessibility, module ref, and native view ref edits patch live on the sample host without reinstall or relaunch. | Added `valdi_rust_hot_reload`, typed patch parser/generator/validator, persistent sample host, exact rebuild diagnostics, module/native-view ref checks, and deterministic latency report. |
| 12 | Complete | Supported action-body edit patches live. | Added `valdi_rust_hot_patch`, action-body edit detector, dev action implementation loader, exact unsupported edit diagnostics, dev-server messages, release exclusion proof, trace snapshot, and support matrix snapshot. |
...
| 05 | Codec validator tests | `bazelisk test //valdi_rust:codec_validator_tests` | Passed | JSON and postcard round-trip, validator, version compatibility, and inspect snapshot tests passed |
| 05 | Codec crate and CLI build | `bazelisk build //valdi_rust/codec:codec //valdi_rust/cli:cli` | Passed | Codec crate and `ir-inspect` CLI compile through Bazel |
| 06 | Backend operation tests | `bazelisk test //valdi_rust:backend_operation_tests` | Passed | Backend contract coverage, trait compile, capability validator, and mock snapshot tests passed |
| 06 | Backend host build | `bazelisk build //valdi_rust/backend:backend` | Passed | Backend crate compiled for the host Rust target |
| 06 | Backend iOS Rust target build | `bazelisk build //valdi_rust/backend:backend --platforms=//bzl/platforms/os:ios_arm64` | Passed | Backend crate compiled for the configured iOS Rust target |
| 06 | Backend Android Rust target build | `bazelisk build //valdi_rust/backend:backend --platforms=//bzl/platforms/os:android_arm64` | Passed | Backend crate compiled for the configured Android Rust target |
| 06 | Rust WASM compile proof | Repo/toolchain inspection | Blocked by missing Rust WASM target/toolchain | `MODULE.bazel` and `bzl/workspace_rust_init.bzl` configure host, iOS, and Android Rust triples; the only WASM condition found is `//bzl/conditions:wasm`, which is not a Rust backend toolchain label |
| 07 | Runtime tree diff tests | `bazelisk test //valdi_rust:runtime_tree_diff_tests` | Passed | Runtime document loader, tree diff ordering, identity patch, rebuild diagnostic, and static backend-op snapshot tests passed |
| 07 | Runtime crate build | `bazelisk build //valdi_rust/runtime:runtime` | Passed | Runtime crate compiles typed document, identity, diff, patch, and static fixture modules |
| 08 | State bindings actions tests | `bazelisk test //valdi_rust:state_bindings_actions_tests` | Passed | State store, binding evaluator, action scheduler, event-action binding, state patch compatibility, and trace snapshot tests passed |
| 08 | Runtime crate build | `bazelisk build //valdi_rust/runtime:runtime` | Passed | Runtime crate compiles typed state, bindings, expressions, actions, scheduler, event bindings, and state fixtures modules |
| 09 | Rust UI DSL tests | `bazelisk test //valdi_rust:rust_ui_dsl_tests` | Passed | DSL golden, invalid diagnostic, Rust app sample compile, no-forbidden-dependency, crate graph, and public API lint tests passed |
| 09 | DSL crate build | `bazelisk build //valdi_rust/dsl:dsl` | Passed | `valdi_rust_dsl` compiles as a Rust app-facing DSL crate with an IR-only crate dependency |
| 10 | TSX normalized IR contract tests | `bazelisk test //valdi_rust:tsx_to_ir_compat_tests` | Passed | 4/4 tests passed: contract/fixture manifest coverage, Rust app no-TS-dependency, crate graph, and public API lint |
| 10 | Companion TS sidecar build | `bazelisk build //compiler/companion:lib` | Passed | TypeScript compatibility sidecar compiled through the existing companion Bazel target |
| 10 | Companion sidecar runtime proof | `NODE_PATH=bazel-bin/compiler/companion/node_modules node <inline PR10 sidecar check>` | Passed | Built JS emitted 22 normalized IR envelopes, exact unsupported-row diagnostic, retained JSXProcessor output, and manifest validator drift detection |
| 10 | Retained TS sample build | `bazelisk build //apps/cli_example:ios.debug.valdimodule` | Passed | Manager-side verification produced `bazel-out/darwin_arm64-opt-exec-ST-d57f47055a04/bin/apps/cli_example/ios/debug/assets/cli_example.valdimodule`; existing Bzlmod, npm directory dependency-check, Swift, and C++ warnings appeared |
| 10 | Companion npm/Jest command | `npm test -- TSXToIRCompatibility.spec.ts TSXFeatureCoverageManifest.spec.ts JSXProcessor.spec.ts` | Blocked by missing local `node_modules` | Environment limitation only: existing project command resolves to `jest --no-watchman`, but this worktree has no `compiler/companion/node_modules/.bin/jest`; Bazel typecheck, built-JS runtime proof, and retained `.valdimodule` build provide PR10 proof in this environment |
| 11 | IR hot reload tests | `bazelisk test //valdi_rust:ir_hot_reload_tests` | Passed | Parser, patch generator, live pipeline/sample host, rebuild diagnostics, module/native view refs, binding/action compatibility, latency report, sample-host snapshot, crate graph, public API lint, and Rust no-TS-dependency tests passed |
| 11 | Hot reload crate build | `bazelisk build //valdi_rust/hot_reload:hot_reload` | Passed | `valdi_rust_hot_reload` compiles against Rust IR, backend, runtime, and DSL metadata without platform renderer code |
| 11 | Hot reload latency report | `//valdi_rust/hot_reload:latency_report_test` | Passed | Deterministic latency snapshot totals 8,900us against a 16,000us patch budget |
| 11 | Rebuild-required diagnostics | `//valdi_rust/hot_reload:rebuild_required_diagnostic_test //valdi_rust/hot_reload:module_native_view_ref_test //valdi_rust/hot_reload:binding_action_compatibility_test` | Passed | Unsupported action-body patch, missing binding/action refs, and module/native-view contract-shape drift return exact code/path/severity/compatibility/source-span diagnostics |
| 12 | Rust hot patch tests | `bazelisk test //valdi_rust:rust_hot_patch_tests` | Passed | Action-body detector, live patch loader, unsupported edit classification, dev-server messaging, release exclusion, support matrix, crate graph, public API lint, and Rust no-TS-dependency tests passed |
| 12 | Hot patch crate build | `bazelisk build //valdi_rust/hot_patch:hot_patch` | Passed | `valdi_rust_hot_patch` compiles against Rust IR, runtime action types, and PR11 action-body patch intent with no new external dependencies |
| 12 | Release exclusion proof | `//valdi_rust/hot_patch:release_exclusion_test` | Passed | Release report excludes hot patch machinery and dev loader symbols, and release loader rejects action-body patches with exact diagnostics |
| 12 | Support matrix proof | `//valdi_rust/hot_patch:support_matrix_snapshot_test //valdi_rust/hot_patch:unsupported_edit_classification_test` | Passed | Only `action_body` is live-patchable; signature, type, module, state shape, dependency, macro, crate graph, and platform-boundary edits return exact rebuild-required diagnostics |
| 09 | Crate graph and public API tests | `bazelisk test //valdi_rust/tests:crate_graph_visibility_test //valdi_rust/tests:forbidden_public_api_test` | Passed | PR09 crate graph metadata includes `valdi_rust_dsl` and public API lint scans `//valdi_rust/dsl:dsl_sources` |
| 09 | Crate graph JSON | `python3 -m json.tool valdi_rust/crate_graph.json` | Passed | `valdi_rust/crate_graph.json` remains valid JSON after adding PR09 metadata |
